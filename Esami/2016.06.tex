...
Si scriva una funzione \sml{hist} (avente tipo \sml{real list -> real * real -> int}) che riceve come argomento una lista di \sml{real l} ed una coppia di \sml{real (c, d)}.
La funzione \sml{hist} ritorna il numero di elementi della lista compresi nell'intervallo \sml{(c - d, c + d)}, estremi esclusi (vale a dire il numero di elementi \sml{r} tali che \sml{(c - d < r < c + d)}.
